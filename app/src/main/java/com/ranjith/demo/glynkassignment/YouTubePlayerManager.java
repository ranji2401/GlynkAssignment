...
import static com.ranjith.demo.glynkassignment.YouTubePlayerDialog.newInstance;
...
class YouTubePlayerManager implements YouTubePlayerHelper.Callback {
  ...
  private YouTubePlayerDialog.FullscreenRequestType fullscreenRequest = null;
  private YouTubePlayerDialog.InitData initData;

  void onSaveState(@NonNull Bundle outState, @Nullable YouTubePlayerDialog.InitData data, boolean configChange) {
    if (fullscreenRequest == null) {
      if (configChange && data != null) {
        fullscreenRequest = YouTubePlayerDialog.FullscreenRequestType.CONFIG_CHANGE;
        this.initData = data;
      }
    }
  ...
  void onRestoreState(@NonNull Bundle savedState, boolean shouldFullScreen) {
    YouTubePlayerDialog.FullscreenRequestType fullscreenRequested =
        (YouTubePlayerDialog.FullscreenRequestType) savedState.getSerializable(STATE_KEY_FULLSCREEN);
    if (fullscreenRequested != null && shouldFullScreen) {
      ...
      playerDialog = newInstance(new YouTubePlayerDialog.InitData(videoOrder, videoId, playbackInfo, orientation));
  ...
  public void onFullscreen(YouTubePlayerHelper helper, YouTubePlayer player, boolean fullscreen) {
    ...
    if (fullscreen) fullscreenRequest = YouTubePlayerDialog.FullscreenRequestType.USER_REQUEST;
    if (fullscreenRequest != null) {
      this.initData = new YouTubePlayerDialog.InitData(helper.getToroPlayer().getPlayerOrder(), helper.getVideoId(),
